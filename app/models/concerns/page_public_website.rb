...
concern :PagePublicWebsite do
  ...
  class_methods do
    ...
    def public_website_page_ids(reload = false)
      ...
      @public_website_page_ids ||= (Page.flagged(:root).pluck(:id) + Page.flagged(:root).first.descendant_page_ids - Page.flagged(:intranet_root).pluck(:id) - Page.flagged(:intranet_root).first.descendant_page_ids)
    end
