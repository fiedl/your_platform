...
class Notification < ActiveRecord::Base
  attr_accessible :recipient_id, :author_id, :reference_url, :reference_type, :reference_id, :message, :text, :sent_at, :read_at, :failed_at if defined? attr_accessible
  ...
  def self.create_from_post(post, options = {})
    ...
    recipients.collect do |group_member|
      if Ability.new(group_member).can? :read, post
  ...
  def self.create_from_comment(comment, options = {})
    ...
    recipients.collect do |recipient|
      if Ability.new(recipient).can? :read, comment
  ...
  def self.create_from_mention(mention, options = {})
    ...
    recipients.collect do |recipient|
      if Ability.new(recipient).can? :read, mention.reference
  ...
  def self.create_from_status_workflow(workflow, target_user, current_user)
    recipients = target_user.responsible_admins
    recipients.collect do |recipient|
  ...
  def self.upcoming_by_user(user)
    ...
    ids = self.upcoming.where(recipient_id: user.id).select do |notification|
      ability.can? :read, notification.reference
    end.map(&:id)
    self.upcoming.where(id: ids) # to keep it an ActiveRecord::Relation
  end
