class GroupMembersController < ApplicationController
  ...
  def load_and_authorize_resource
    ...
    load_members_without_memberships
  ...
  # For special groups that override `members`, but not `memberships`.
  def load_members_without_memberships
    if @memberships.none? && @group.members.any?
      @memberships = nil
      @members = @group.members.select { |user| can? :read, user }
    end
  end
