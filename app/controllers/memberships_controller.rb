class MembershipsController < ApplicationController
  ...
  def index
    if params[:user_id]
      ...
      @memberships = Membership.where(user: @user).now_and_in_the_past.direct.to_a
    elsif params[:group_id]
      @object = @group = Group.find(params[:group_id])
      authorize! :manage, @group
      
      @memberships = Membership.where(group: @group).now_and_in_the_past.direct.to_a
    end
  ...
  def destroy
    @membership.try(:destroy) && head(:no_content)
  end
  ...
  def create
    if membership_params[:user_title].present?
      ...
      begin 
        @membership = UserGroupMembership.create(membership_params.merge({user_id: @user_id}))
  ...
  def membership_params
    if can? :manage, @membership
      params.require(:membership).permit(:valid_to, :valid_from, :user_title, :user_id, :group_id, :id,
        :valid_from_localized_date, :valid_to_localized_date,
        :needs_review, :ancestor_id, :ancestor_type, :descendant_id, :descendant_type)
