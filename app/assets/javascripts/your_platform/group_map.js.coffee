$(document).ready ->
  ...
  App.process_group_maps($('body'))
  App.animate_group_map_items()


App.process_group_maps = (element)->
  $(element).find('.group_map').each ->
    group_map = $(this)

    # scale down the map for smaller devices
    if group_map.width() < 350
      ratio = 0.7
      group_map.find('.group_map_background')
        .width(group_map.find('.group_map_background').width() * ratio)
        .height(group_map.find('.group_map_background').height() * ratio)
    ...
    map_width = group_map.find('.group_map_background').width()
    map_height = group_map.find('.group_map_background').height()
    map_longitude_width = map_right_longitude - map_left_longitude
    map_latitude_height = map_top_latitude - map_bottom_latitude

    group_map.find('.map_item').each ->
      map_item = $(this)
